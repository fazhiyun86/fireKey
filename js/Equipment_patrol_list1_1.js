...
mui.plusReady(function() {
	...
	function WebApp_GetISMTaskGroup(Id) {
		var urlTask = 'http://' + localStorage.getItem("serverAddress") + ':' + localStorage.getItem("portNum") + '/WebApi/DataExchange/GetData/WebApp_GetISMTaskGroup?dataKey=00-00-00-00&TaskID=' + Id;
				console.log(urlTask);
		mui.ajax(urlTask, {
			data: null,
			dataType: 'json', //返回
			...
			success: function(data) {
				...
				mui.each(getDatas, function(index, item) {
					html +=
						...
						item.TaskGroupName +
						'</p>' +
						'<p class="Equipment_patrol_list1_1_data"><span>巡检单位：' +
						item.TaskGroupName +
						'</span><span>完成检查：' +
