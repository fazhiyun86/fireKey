...
mui.plusReady(function() {
	...
	function WebApp_GetEquipmentISMList() {
		var code = localStorage.getItem('userNumCode');
		var url = 'http://' + localStorage.getItem("serverAddress") + ':' + localStorage.getItem("portNum") + '/WebApi/DataExchange/GetData/WebApp_GetEquipmentISMList?dataKey=00-00-00-00&Code=' + code;
		console.log(url);
		var htmlDates = '';
		mui.ajax(url, {
			data: null,
			...
			success: function(data) {
				...
				mui.each(getDatas1, function(index, item) {
					...
					htmlDates +=
						...
						'</p>' +
						'<p  style="white-space:nowrap; ">' +
						'检查结果：' + IsNormal[item.IsNormal] + 
						'</p>' +
						'<p>' +
						'任务状态：' + InspectStatus[item.InspectStatus]+
						'</p>' +
						'</div>' +
						'<div class="Equipment_state_query_page3_con2_1_left">' +
						'<p>' +
