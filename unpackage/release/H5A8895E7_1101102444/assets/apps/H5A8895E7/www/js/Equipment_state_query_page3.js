...
mui.plusReady(function() {
	...
	function WebApp_GetEquipmentrepairList() {
		var code = localStorage.getItem('userNumCode');
		var url = 'http://' + localStorage.getItem("serverAddress") + ':' + localStorage.getItem("portNum") + '/WebApi/DataExchange/GetData/WebApp_GetEquipmentrepairList?dataKey=00-00-00-00&Code=' + code;
		console.log(url);
		var htmlDates = '';
		mui.ajax(url, {
			data: null,
			...
			success: function(data) {
				...
				mui.each(getDatas1, function(index, item) {
					...
					htmlDates +=
						...
						'</p>' +
						'<p>' +
						'处理状态：' + RepairResult[item.RepairResult] +
						'</p>' +
						'</div>' +
						'<div class="Equipment_state_query_page3_con2_1_left">' +
						'<p>' +
						...
						'</p>' +
						'<p>' +
						'维修评价：' + item.AppraiseResult +
						'</p>' +
						'<p>' +
						'维修时长：' + item.ReportLength +
						'</p>' +
						'</div>' +
						'</div>';
				})
